...
###############################################################################
FolderCF = paste(FolderRoot, "/R3-Config-Files", sep="")
...
while(i<=n){
  ...
  # Confi File Name
  file_name = paste(FolderCF, "/R3-", ds$Name, ".csv", sep="")
  ...
  write("Dataset_Path, ~/Generate-Partitions-Random3/Datasets",
        file = output.file, append = TRUE)

  # job name
  job_name = paste("R2-", ds$Name, sep = "")
